...
function prosesCallBackQuery($message)
{
    ...
    if ($data == '1' || $data == '2' || $data == '3') {
        ...
        if ($data == 'RNA' || $data == 'ALAMAT' || $data == 'PENDING') {
            $jenis_kendala = 'P';
        }
        elseif ($data == 'BATAL') {
            $jenis_kendala = 'B';
        ...
        $update = mysqli_query($koneksi,"UPDATE tb_kendala SET jenis_kendala='$jenis_kendala', kendala='$data' WHERE message_id = $meidd");
        if ($update) {
            $lihat = mysqli_query($koneksi,"SELECT * FROM tb_kendala WHERE message_id = '$meidd'");
            if (mysqli_num_rows($lihat)>0){
                while($d = mysqli_fetch_array($lihat)){
                    $sales_id = $d['sales_id'];
                    $update = mysqli_query($koneksi,"UPDATE tb_sales SET status='K$jenis_kendala', status_id='6' WHERE sales_id = $sales_id");
                    $savelog = mysqli_query($koneksi,"INSERT INTO tb_log (sales_id, action_by, action_on, action_status, a_keterangan) VALUES ($sales_id, '$fromname', '$time', 6, '$data')");
...
function prosesPesanTeks($message)
{
    ...
    $fromname   = $message["from"]["first_name"];
    $grupname   = $message["chat"]["title"];
    $str        = strtok($message["reply_to_message"]["text"], "\n");
    $replyp     = '!create data';
    ...
    if ($masuk_switch_r) {
        ...
        switch (true) {
            ...
            case $reply == '👥 Masukan KODE DATEL :':
            	$pesan = strtoupper($pesan);
                if (($pesan == 'PKL') || ($pesan == 'SLW') || ($pesan == 'BTG') || ($pesan == 'TEG') || ($pesan == 'BRB') || ($pesan == 'PML')) {
                    ...
                    $text = "KODE DATEL yang kamu masukan salah. Contoh KODE DATEL: PKL, SLW, BTG, TEG, BRB, PML";
                    sendApiMsg($chatid, $text);
                    $text = '👥 Masukan KODE DATEL :';
                    sendApiMsgReply($chatid, $text);
                }
            ...
            case $str   == '!create data':
                if ($pesan == 'wait') {
                    ...
                    $cekdata      = mysqli_query($koneksi,"SELECT status_id FROM tb_provisioning WHERE message_id=$replymid AND group_id=$replygid");
            ...
            case $reply == 'Oke sekarang tulis keterangan tentang kendala ini:':
                $query = mysqli_query($koneksi,"UPDATE tb_kendala SET keterangan='$pesan' WHERE message_id = $meidd");
                sendApiAction($chatid);
                if ($query) {
                    ...
                    $text = "❗️ Maaf laporan gagal dilakukan. Silahkan coba beberapa saat lagi.. $meidd";
                    sendApiMsg($chatid, $text);
                }
                break;
            ...
            case $str == 'ORDER':
                ...
                if ($pesan == '/laporoldd') {
                    ...
                    $inkeyboard = [
                        [
                            ['text' => 'A. RNA', 'callback_data' => 'RNA'],
                            
                        ],
                        [
                            ['text' => 'B. ALAMAT', 'callback_data' => 'ALAMAT'],
                        ],
                        [
                            ['text' => 'C. BATAL', 'callback_data' => 'BATAL'],
                        ],
                        [
                            ['text' => 'D. PENDING', 'callback_data' => 'PENDING'],
                        ],
                        [
                            ['text' => 'E. ODP FULL', 'callback_data' => 'ODP FULL'],
                        ],
                        [
                            ['text' => 'F. ODP LOSS', 'callback_data' => 'ODP LOSS'],
                        ],
                        [
                            ['text' => 'G. ODP RETI', 'callback_data' => 'ODP RETI'],
                        ],
                        [
                            ['text' => 'H. TIANG', 'callback_data' => 'TIANG'],
                        ],
                        [
                            ['text' => 'I. PT2', 'callback_data' => 'PT2'],
                        ],
                        [
                            ['text' => 'J. NO FO/ODP', 'callback_data' => 'NO FO/ODP'],
                        ],
                        [
                            ['text' => 'K. RUTE INSTALASI', 'callback_data' => 'RUTE INSTALASI'],
                        ],
                        [
                            ['text' => 'L. CROSING JALAN', 'callback_data' => 'CROSING JALAN'],
                        ]
                    ];
                    ...
                    $cekid      = mysqli_query($koneksi,"SELECT sales_id,kendala FROM tb_kendala WHERE sales_id = '$sales_id'");
                    ...
                    if (mysqli_num_rows($cekid)>0){
                        ...
                        $query = mysqli_query($koneksi,"INSERT INTO tb_kendala (sales_id,post_by,post_on,last_update,message_id) VALUES ('$sales_id','$fromid','$datenow','$datenow','$meid')");
                        if ($query) {
                ...
                elseif (strtok($pesan, "\n") == '/reqsc' || strtok($pesan, "\n") == '/reqsc ') {
                    ...
                    $cekid    = mysqli_query($koneksi,"SELECT status_id,sales_id FROM tb_sales WHERE sales_id = '$sales_id' AND status_id > 2");
                    ...
                    if (mysqli_num_rows($cekid)>0){
	                        ...
	                        elseif (strlen($dc) < 4) {
	                            ...
	                            if ($query) {
                                    ...
                                    while($d = mysqli_fetch_array($ceksales)){
                                        ...
                                        $s_m_id = $d['message_id'];
                                        while($d = mysqli_fetch_array($ambilt)){
                                            $savelog = mysqli_query($koneksi,"INSERT INTO tb_log (sales_id, action_by, action_on, action_status) VALUES ('$sales_id','$d[nama_teknisi]','$datenow',3)");
                                            $pesantosales = "⚠️ Order dimintakan SC oleh $t[nama_teknisi]";
                                            sendApiMsg($s_telegram_id, $pesantosales, $s_m_id, 'Markdown');
                                        }
                                    }
	                                $text = '✅ Request SC berhasil dilakukan, silahkan tunggu.';
	                            }
	                            else{
	                                $text = "❗️ Maaf request sc gagal dilakukan. Silahkan coba beberapa saat lagi..";
	                            }
	                        }
                        }
                        // if (mysqli_num_rows($cekorder)>0) {
                        ...
                        // }
                        // else{
                        //     $text = "❌ Order ID JA$sales_id tidak dalam status terorder ke teknisi!";
                        //     sendApiMsg($chatid, $text);
                        // }
                    }
                ...
                elseif (strtok($pesan, "\n") == '/order') {
                    sendApiAction($chatid);
                    $text = '❗️ Maaf fitur dalam pengembangan. Anda dapat mengirim order ke teknisi melalui aplikasi web https://sales.jarvisid.com';
                    sendApiMsg($chatid, $text, false, 'Markdown');
                    // $pesan = str_replace('/order', '', $pesan);
                    // $reply_message  = $message["reply_to_message"]["text"];
                    // $reply_message  = preg_replace('/^.+\n/', '', $reply_message);
                    // $sales_id       = str_replace('JA', '', strtok($reply_message, "\n"));

                    // $cekid      = mysqli_query($koneksi,"SELECT sales_id FROM tb_sales WHERE sales_id = '$sales_id'");
                    // $cekorder   = mysqli_query($koneksi,"SELECT sales_id FROM tb_sales WHERE sales_id = '$sales_id' AND status_id = 1");
                    // $datenow 	= date('Y-m-d H:i:s');
                    // $meid    	= $message["message_id"];
                    // if (mysqli_num_rows($cekid)<0){
                    //     $text = '❗️ Order ID JA'.$sales_id.' tidak ditemukan!';
                    //     sendApiMsg($chatid, $text, false, 'Markdown');
                    // }
                    // else{
                    //     if (mysqli_num_rows($cekorder)>0) {
                    //          $query = mysqli_query($koneksi,"UPDATE tb_sales SET tgl_progress = '$datenow', status = 'order', status_id = '6', keterangan = 'PROGRESS BY $fromname, PROGRESS TO $pesan' WHERE sales_id=$sales_id");                            
                    //          if ($query) {
                    //             $text = '✅ Status order berhasil dirubah menjadi ORDERED.';
                    //             sendApiMsg($chatid, $text, false, 'Markdown');
                    //         }
                    //         else{
                    ...
                    //     }
                    //     else{
                    //         $text = "❌ JA$sales_id ordernya tidak sedang waiting!";
                    //         sendApiMsg($chatid, $text, false, 'Markdown');
                    //     }  
                    // }
                }
    ...
    if ($masuk_switch_m) {
        if ($type_ms == "private") {
            switch (true) {
                ...
                case $pesan == '/help':
                    sendApiAction($chatid);
                    $text = "Halo. kenalin saya jarvis, asisten nya tim DAMAN PEKL, saya ditugaskan untuk membantu pekerjaan teman-teman semua.\n\n";
                    ...
                    $text .= "/reqsc [enter] ODP [enter ] PORT [enter] DC untuk request SC ke HD.\n";
                    $text .= "/lapor untuk lapor order yang terkendala.\n";
                    $text .= "!create untuk mengirim order provisioning siap create (hanya untuk digrup).\n";
                    ...
                    $text .= "reply order provisioning dengan jawaban 'wait' untuk mengambil order tersebut. (hanya untuk digrup)\n\n";
                    $text .= "😎 Terima Kasih";
                    sendApiMsg($chatid, $text);
                    break;
                ...
                case preg_match("/\/laporold (.*)/", $pesan, $hasil):
                    ...
                    $sc      = intval(preg_replace('/[^0-9]+/', '', $hasil[1]), 10);
                    $ceksc   = mysqli_query($koneksi,"SELECT sales_id FROM tb_kendala WHERE sales_id = '$sc'");
                    $datenow = date('Y-m-d H:i:s');
                    $meid    = $message["message_id"];
                    if (mysqli_num_rows($ceksc)>0){
                        ...
                        $query = mysqli_query($koneksi,"INSERT INTO tb_kendala (sales_id,post_by,post_on,last_update,message_id) VALUES ('$sc','$fromid','$datenow','$datenow','$meid')");
                        if ($query) {
                            sendApiKeyboard($chatid, 'Silahkan pilih jenis kendala nya :', $inkeyboard, true);
                        }
                        else{
                            ...
                            if (stripos($arrpsn[1], 'PELANGGAN') == false OR stripos($arrpsn[1], ':') == false) {
                                ...
                                elseif ($ceklat[2] != '.') {
                                    ...
                                    if (mysqli_num_rows($cekdata)>0){
                                        ...
                                        else if (strposa($odp, $dpkl) !== false) {
                                            $datel = 'PKL';
                                        }
                                        ...
                                        if ($datel == 'none') {
                                            ...
                                            $query = mysqli_query($koneksi,"INSERT INTO tb_sales (sales_id, datel, nama_pelanggan, no_ktp, alamat, lat_long, cp, email, odp, kode, paket, jarak_tiang, myir, kategori, isi_pesan, message_id, message_from, tgl_post, tgl_update, status) VALUES (NULL, '$datel', '$plgn', '$no_ktp', '$alamat', '$lat_long', '$cp', '$email', '$odp', '$kode', '$paket', '$jarak_tiang', '$myir', '$kategori', '$pesan','$post_id', '$fromid', '$tgl','$tgl','$status')");
                                            $idsales = mysqli_insert_id($koneksi);
                                            if ($query) {
                                                $ambil = mysqli_query($koneksi,"SELECT * FROM  tb_sales s JOIN tb_salesman u ON u.s_telegram_id = s.message_from WHERE sales_id = '$idsales'");
                                                if (mysqli_num_rows($ambil)>0){
                                                    while($d = mysqli_fetch_array($ambil)){
                                                        if ($d['kategori'] == 1) {
                                                            $kategori = 'DEAL, ODP READY';
                                                        ...
                                                        $savelog = mysqli_query($koneksi,"INSERT INTO tb_log (sales_id, action_by, action_on, action_status) VALUES ($d[sales_id], '$d[fullname]', '$tgl', 1)");
                                                        if ($d['kategori'] == 1) {
                                                            $dtl = $d['datel'];
            ...
            switch (true) {
                ...
                case $pesan == '/showns@damanasisten_bot':
                    if ($chatid != '-1001303948672') {
                        sendApiAction($chatid);
                        $image_data = file_get_contents("https://api.apiflash.com/v1/urltoimage?access_key=4e8151c3b7104ea28346bfa76e9b4907&height=768&fresh=true&url=https%3A%2F%2Fprovisioning.jarvisid.com%2Fprovisioning%2Fshow%2Fnew_sales&width=1024");
                ...
                case $pesan == '/showmig@damanasisten_bot':
                    if ($chatid != '-1001303948672') {
                        sendApiAction($chatid);
                        $image_data = file_get_contents("https://api.apiflash.com/v1/urltoimage?access_key=4e8151c3b7104ea28346bfa76e9b4907&height=768&fresh=true&url=https%3A%2F%2Fprovisioning.jarvisid.com%2Fprovisioning%2Fshow%2Fmigrasi&width=1024");
                ...
                case $pesan == '/showaddon@damanasisten_bot':
                    if ($chatid != '-1001303948672') {
                        sendApiAction($chatid);
                        $image_data = file_get_contents("https://api.apiflash.com/v1/urltoimage?access_key=4e8151c3b7104ea28346bfa76e9b4907&height=768&fresh=true&url=https%3A%2F%2Fprovisioning.jarvisid.com%2Fprovisioning%2Fshow%2Fadd_on&width=1024");
                ...
                case $pesan == '/help@damanasisten_bot':
                    ...
                    // $text .= "!lapor [sc] untuk lapor kendala provisioning.\n";
                    // $text .= "/regsales untuk registrasi sebagai Sales.\n";
                    $text .= "/showns untuk melihat perolehan order new sales hari ini.\n";
                    $text .= "/showmig untuk melihat perolehan order migrasi hari ini.\n";
                    $text .= "/showaddon untuk melihat perolehan order addon hari ini.\n";
                ...
                case $pesan == '/order@damanasisten_bot':
                    ...
                    switch ($chatid) {
					    case -263944966:
					        $ambilwait = mysqli_query($koneksi,"SELECT * FROM  tb_sales WHERE status_id = 1 AND datel = 'PKL' AND kategori = 1");
					        break;
					    case -341677349:
					        $ambilwait = mysqli_query($koneksi,"SELECT * FROM  tb_sales WHERE status_id = 1 AND datel = 'BRB' AND kategori = 1");
					        break;
					    case -280898518:
					        $ambilwait = mysqli_query($koneksi,"SELECT * FROM  tb_sales WHERE status_id = 1 AND datel = 'BTG' AND kategori = 1");
					        break;
					    case -338613303:
					        $ambilwait = mysqli_query($koneksi,"SELECT * FROM  tb_sales WHERE status_id = 1 AND datel = 'TEG' AND kategori = 1");
					        break;
					    case -336766109:
					        $ambilwait = mysqli_query($koneksi,"SELECT * FROM  tb_sales WHERE status_id = 1 AND datel = 'SLW' AND kategori = 1");
					        break;
					    case -371367732:
					        $ambilwait = mysqli_query($koneksi,"SELECT * FROM  tb_sales WHERE status_id = 1 AND datel = 'PML' AND kategori = 1");
					        break;
                ...
                case preg_match("/\!createkunci (.*)/", $pesan, $hasil):
                    sendApiAction($chatid);
                    /*bersih bersih pesan*/
                    $pesan = strtoupper($message['text']);
                    ...
                    preg_match('/ODP REAL\s{0,1}:(.+)/i', $pesan, $odp);
                    ...
                    $sclam      = (trim($sclam[1]) != '' ? trim($sclam[1]) : '-');
                    $breket     = (trim($breket[1]) != '' ? trim($breket[1]) : '-');
                    $ros        = (trim($ros[1]) != '' ? trim($ros[1]) : '-');
                    $t7         = (trim($t7[1]) != '' ? trim($t7[1]) : '-');
                    $cp         = (trim($cp[1]) != '' ? trim($cp[1]) : '-');
                    $stbid      = (trim($stb_id[1]) != '' ? trim($stb_id[1]) : '-');
                    $spl12      = (trim($spl12[1]) != '' ? trim($spl12[1]) : '-');
                    $spl14      = (trim($spl14[1]) != '' ? trim($spl14[1]) : '-');
                    $spl18      = (trim($spl18[1]) != '' ? trim($spl18[1]) : '-');
                    $cassete    = (trim($cassete[1]) != '' ? trim($cassete[1]) : '-');
                    $adapter    = (trim($adapter[1]) != '' ? trim($adapter[1]) : '-');
                    $utp        = (trim($utp[1]) != '' ? trim($utp[1]) : '-');
                    $rj45       = (trim($rj45[1]) != '' ? trim($rj45[1]) : '-');
                    // variable pelengkap
                    $odp        = str_replace(' ', '', $odp);
                    $tgl        = date('Y-m-d H:i:s');
                    ...
                    $cekta      = mysqli_query($koneksi,"SELECT t_telegram_id,active FROM tb_teknisi WHERE t_telegram_id = '$fromid' AND active = 1");
                    ...
                    if (mysqli_num_rows($cekut)>0){
                        if (mysqli_num_rows($cekta)>0){
                            if (mysqli_num_rows($cekscsales)>0){
                                if (mysqli_num_rows($ceksc)>0){
                                    ...
                                    if (strlen($odp) < 14 || strlen($odp) > 15) {
                                        $text = "ODP tidak valid! tulis ODP dengan ODP : ODP-PKL-FAA/001";
                                    }
                                    else if (strpos($odp, '/') == false) {
                                        $text = "tanda / nya mana? misal : ODP-PKL-FAA/001";
                                    }
                                    else if ( (mysqli_num_rows($cekbc)<=0) && (strpos($order, 'MIG') !== false || strpos($order, 'PSB') !== false) ) {
                                        $text = "Barcode $bc tidak valid, silahkan perbaiki/ganti barcode nya!";
                                    }
                                    else if ( (mysqli_num_rows($cekbcpakai)>0) && (strpos($order, 'MIG') !== false || strpos($order, 'PSB') !== false) ) {
                                        $text = "Barcode $bc sudah digunakan, silahkan ganti yang lain ya..";
                                    }
                                    else{
                                        $query = mysqli_query($koneksi,"INSERT INTO tb_provisioning (datel, order_type, atas_nama, alamat, cp, voice, internet, odp, port, sisa, sn, sc, sc_baru, mitra, teknisi, crew, barcode, stb_id, post_by, group_id, message_id, tgl_masuk) VALUES ('$datel', '$order', '$an', '$alamat', '$cp', '$telp', '$inet', '$odp', '$port', '$sisa', '$sn', '$sc', '$sc', '$mitra', '$teknisi', '$crew', '$bc', '$stbid', '$fromid', '$g_id','$m_id', '$tgl')");
                                        $idcreate = mysqli_insert_id($koneksi);
                                        if ($query) {
                                            $insmtr = mysqli_query($koneksi,"INSERT INTO tb_material (create_id, dropcore, sclam, breket, ros, t_tujuh, spl1_2, spl1_4, spl1_8, cassete, adapter, utp, rj45) VALUES ('$idcreate', '$dc', '$sclam', '$breket', '$ros', '$t7', '$spl12', '$spl14', '$spl18', '$cassete', '$adapter', '$utp', '$rj45')");
                                            if ($insmtr) {
                                                ...
                                                while($d = mysqli_fetch_array($cekscsales)){
                                                    ...
                                                    while($t = mysqli_fetch_array($ambilt)){
                                                        ...
                                                        $udatasales = mysqli_query($koneksi,"UPDATE tb_sales SET status = 'hr', status_id = '7'  WHERE sales_id=$sales_id");
                                ...
                                $text = '❌ SC tidak valid! Mohon Cek lagi nomor SC nya!';
                            }
                        }
                        else{
                          $text = 'Maaf akun anda belum di approve oleh admin. Silahkan hubungi admin untuk aktifasi akun telegram anda.'; 
                        }
                    }
                    else{
                        $text = 'Anda belum terdaftar sebagai teknisi. silahkan lakukan pendaftaran dengan perintah /regteknisi';
                    }
                ...
                case preg_match("/\!create (.*)/", $pesan, $hasil):
                    ...
                    $utp        = (trim($utp[1]) != '' ? trim($utp[1]) : '-');
                    $rj45       = (trim($rj45[1]) != '' ? trim($rj45[1]) : '-');
                    $redaman    = (trim($redaman[1]) != '' ? trim($redaman[1]) : '-');
                    // variable pelengkap
                    $odp        = str_replace(' ', '', $odp);
                    $tgl        = date('Y-m-d H:i:s');
                    $post_name  = $message["from"]["username"];
                    ...
                    $an         = $an == '-' ? '-' : preg_replace('/[^\da-z ]/i', '', $an);
                    $alamat     = $alamat == '-' ? '-' : preg_replace('/[^\da-z ]/i', '', $alamat);
                    ...
                    if (mysqli_num_rows($ceksc)>0){
                        ...
                        else if ($mitra != 'HCP' && $mitra != 'KOPEGTEL' && $mitra != 'NUTEL' && $mitra != 'KJS' && $mitra != 'KOPTA' && $mitra != 'GLOBAL' && $mitra != 'ZAG' && $mitra != 'TA' && $mitra != 'GTI' && $mitra != 'APRA'){
                            $text = "⚠️ Perbaiki nama mitra!";
                        }
                                ...
                                if ($insmtr) {
                                    if (mysqli_num_rows($cekut)>0){
                                        ...
                                        while($d = mysqli_fetch_array($cekscsales)){
                                            ...
                                            while($t = mysqli_fetch_array($ambilt)){
                                                $savelog = mysqli_query($koneksi,"INSERT INTO tb_log (sales_id, action_by, action_on, action_status) VALUES ('$sales_id','$t[nama_teknisi]','$tgl',7)");
                                                $udatasales = mysqli_query($koneksi,"UPDATE tb_sales SET status = 'hr', status_id = '7', tgl_update = '$tgl'  WHERE sales_id='$sales_id'");
                                                $pesantosales = "⚠️ Order dimintakan create oleh teknisi $t[nama_teknisi]";
                                                sendApiMsg($s_telegram_id, $pesantosales, $s_m_id, 'Markdown');
                                            }
                                        }
                ...
                case preg_match("/\!inputWMS (.*)/", $pesan, $hasil):
                    if ($chatid == '-273243235' || $chatid == '-423730453') { // -423730453
                        sendApiAction($chatid);
                        $cekus = mysqli_query($koneksi,"SELECT s_telegram_id FROM tb_salesman WHERE s_telegram_id = '$fromid'");
                        $cekhs = mysqli_query($koneksi,"SELECT s_telegram_id,active FROM tb_salesman WHERE s_telegram_id = '$fromid' AND active = 1");
                        /*bersih bersih pesan*/
                                ...
                                if ($datel == 'none') {
                                    ...
                                    if ($query) {
                                        ...
                                        if (mysqli_num_rows($ambil)>0){
                                            ...
                                            $text  = "Data berhasil masuk, namun tidak dapat memberikan informasi order yang barusan dimasukan karena kamu belum register sebagai sales. Mohon register dulu sebagai sales ke @damanasisten_bot kemudian ketik /regsales atau /help untuk infromasi bantuan.";
                                        }
                                    }
                                    else{
                                        $text  = "Maaf data gagal disimpan, silahkan coba beberapa saat lagi..";
